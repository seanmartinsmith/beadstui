...
// Replaced perles internal/log with log/slog.
...
// This is the main entry point for BQL parsing.
...
func (p *Parser) Parse() (*Query, error) {
	...
	if p.current.Type != TokenEOF {
		...
		slog.Debug("BQL parse failed: unexpected token",
...
func (p *Parser) parseFactor() (Expr, error) {
	...
	case TokenLParen:
		...
		if p.current.Type != TokenRParen {
			...
			slog.Debug("BQL parse failed: missing closing paren",
				"token", p.current.Literal,
				"position", p.current.Pos)
			return nil, err
		}
...
func (p *Parser) parseComparison() (Expr, error) {
	...
	if p.current.Type != TokenIdent {
		...
		slog.Debug("BQL parse failed: expected field name",
			"token", p.current.Literal,
			"position", p.current.Pos)
		return nil, err
	}
	...
	if !p.current.Type.IsComparisonOp() {
		...
		slog.Debug("BQL parse failed: expected operator",
			"token", p.current.Literal,
			"position", p.current.Pos)
		return nil, err
	}
...
func (p *Parser) parseInExpr(field string, not bool) (Expr, error) {
	...
	if p.current.Type != TokenLParen {
		err := fmt.Errorf("expected '(' at position %d, got %q", p.current.Pos, p.current.Literal)
		slog.Debug("BQL parse failed: expected '(' for IN expression",
			"field", field,
			"token", p.current.Literal,
			"position", p.current.Pos)
		return nil, err
...
func (p *Parser) parseValue() (Value, error) {
	...
	default:
		...
		slog.Debug("BQL parse failed: expected value",
			"token", p.current.Literal,
			"position", p.current.Pos)
		return v, err
	}
...
func (p *Parser) parseOrderBy() ([]OrderTerm, error) {
	...
	if p.current.Type != TokenOrder {
		...
		slog.Debug("BQL parse failed: expected 'order' keyword",
			"token", p.current.Literal,
			"position", p.current.Pos)
		return nil, err
	}
	...
	if p.current.Type != TokenBy {
		...
		slog.Debug("BQL parse failed: expected 'by' keyword",
			"token", p.current.Literal,
			"position", p.current.Pos)
		return nil, err
	}
	...
	for {
		...
		if p.current.Type != TokenIdent {
			err := fmt.Errorf("expected field name at position %d, got %q", p.current.Pos, p.current.Literal)
			slog.Debug("BQL parse failed: expected field name in ORDER BY",
				"token", p.current.Literal,
				"position", p.current.Pos)
			return nil, err
		}
...
func (p *Parser) parseExpand() (*ExpandClause, error) {
	...
	if p.current.Type != TokenIdent {
		...
		slog.Debug("BQL parse failed: expected expansion type",
			"token", p.current.Literal,
			"position", p.current.Pos)
		return nil, err
	}
	...
	if err != nil {
		...
		slog.Debug("BQL parse failed: invalid expansion type",
			"token", p.current.Literal,
			"position", p.current.Pos,
			"error", err)
