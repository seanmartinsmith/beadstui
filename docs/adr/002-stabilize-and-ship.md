...
**Priority**: P1
**Bead**: bt-0cht
**Foundation**: `docs/audits/domain/2026-04-03-cli-ergonomics-audit.md`, `docs/design/2026-04-20-bt-mhwy-1-compact-output.md`
...
- [ ] **Sub-stream surfaced 2026-04-25: post-Dolt migration of bt-derived data layer.** bt's correlator (`pkg/correlation/`) and sprint loader (`pkg/loader/sprint.go`) were built against the pre-v0.56.1 era of beads (JSONL backups available alongside Dolt). Beads is Dolt-only since v1.0.1. Affected subcommands: `history`, `related`, `causality` (correlator-bound — bt-08sh), `forecast`, `sprint show` (sprint-bound — bt-z5jj). Plus an ADR-flavored decision about canonical `.beads/` vs `.bt/` data-home split (bt-uahv). bt-vhn2 closed as superseded; original "--global routing" framing was wrong. **Extended 2026-04-25 by [ADR-003](003-data-source-architecture-post-dolt.md)** — addresses the SourceType abstraction shape itself. Accepted 2026-04-25 with option (b): collapse to `Dolt | DoltGlobal | JSONLFallback`. Implementation tracked in bt-05zt: **Phase 1 (mechanical SQLite reader removal) landed 2026-05-03** as part of the predict-swarm parallel-stream session — sqlite.go deleted, SourceTypeSQLite removed, comment-sweep included. Phase 2 (abstraction collapse, `metadataPreferredSource` fold) remains open under same bead. Foundation survey: `docs/audits/architecture/2026-04-25-data-source-architecture-survey.md`.
...
**Foundation**: `docs/audits/gaps/2026-04-03-bql-gap-analysis.md`
...
**Foundation**: `docs/audits/domain/2026-04-03-test-suite-audit.md`
...
- `bt-t82t` — Phase 4 cleanup: stale `bv-` references in live tutorial/history/UI text, golden file regen, `go test ./...` + `go vet ./...` validation, verify bt-0cht items naturally resolved by Cobra migration. **Partially advanced 2026-05-03** by bt-uikz (Stream A of predict-swarm session): 95 `bv-` occurrences in `pkg/ui/tutorial.go` (73) and `pkg/ui/board_test.go` (22) swept; AGENTS.md "Key Directories" + filename-claim corrected; `agents.AgentsFileName` constant extracted. The remaining ~1,237 `bv-` references across other files in pkg/ui, pkg/correlation, pkg/baseline etc. stay under bt-t82t.
...
Recent completions (2026-05-03 — predict-swarm Phase 1 landings, parallel worktree dispatch):
- [x] **bt-uikz** (Stream A, P2 task, area:docs): AGENTS.md correctness sweep. Extracted `agents.AgentsFileName` constant (single-source invariant replaces "15 Go files" prose claim). Fixed Key Directories phantom paths (dropped `internal/dolt/`, `internal/models/`; added `internal/doltctl/`, `pkg/view/`). Swept 95 `bv-` sigils from `pkg/ui/{tutorial,board_test}.go`. Closes findings DA-1, DA-4, DA-6 (pkg/ui subset).
- [x] **bt-05zt Phase 1** (Stream B, P2 feature, area:data): Mechanical SQLite reader removal per ADR-003. Deleted `internal/datasource/sqlite.go` (397 LOC). Removed `SourceTypeSQLite` / `PrioritySQLite` / `discoverSQLiteSources`. Updated 9 callers. Comment-sweep follow-up commit cleaned `cobra_tail.go`, `background_worker.go`, `pkg/tail/stream.go`. Closes findings AR-2, code half of DA-3, partial AR-8. Phase 2 (enum collapse, `metadataPreferredSource` fold) remains open.
- [x] **bt-zcs0** (Stream C, **P0 bug, area:cli + security**): Default-deny + TOFU trust DB for `.bt/hooks.yaml` execution. Closes finding SA-1 (CRITICAL — RCE via cloned repos). Trust binds (abs-path, sha256-of-contents) at `~/.bt/hook-trust.json` mode 0600. Trust gate in `Executor.SetTrustGate`; `Loader.Load` stays read-only. Breaking flag flip: `--no-hooks` removed, `--allow-hooks` added (default safe). New `bt hooks list` + `bt hooks trust` subcommands. New SECURITY.md threat model. 9 new tests in `pkg/hooks/trust_test.go`. Documented gap: external scripts not hash-covered.
- [x] All 3 dispatched as parallel worktree subagents in a single PM dispatch; ~30 min wall vs sequential estimate.
- Filed as follow-ups: **bt-xwt5** (P2, `.gitattributes` should pin golden JSON to `eol=lf` — fresh worktrees see CRLF and fail pkg/view tests), **bt-1gp2** (P3, consolidate `handleHookError` + `exitOnHookError` helpers).
...
- `domain/2026-04-03-test-suite-audit.md` - 268 test files categorized
- `domain/2026-04-03-cli-ergonomics-audit.md` - 97 flags inventoried, severity-ranked issues
- `gaps/2026-04-03-bql-gap-analysis.md` - 5 bugs, 3 remaining features, priority order
- `gaps/2026-04-08-global-mode-readiness.md` - readiness audit for cross-project features
- `architecture/2026-03-16-architecture-map.md` - cross-domain dependency graph (session 16)
